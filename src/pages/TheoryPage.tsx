...
import { getTopicById, TOPICS, type Language } from '@/data/curriculum';
...
export default function TheoryPage() {
  ...
  const prerequisites = topic.prerequisites.map(id => TOPICS.find(t => t.id === id)).filter(Boolean);
  const unmetPrereqs = prerequisites.filter(p => p && getTopicStatus(p.id) !== 'completed');
  ...
  const handleMarkComplete = async () => {
    ...
    toast.success(`🎉 ${topic.title} completed! Next topics unlocked.`);
    ...
    <div className="p-6 max-w-4xl mx-auto space-y-6 animate-fade-in">
          ...
          <div className="flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className={`text-xs px-2 py-1 rounded-full ${
                ...
                {status === 'completed' ? '✓ Completed' : status === 'in-progress' ? '▶ In Progress' : status === 'unlocked' ? 'Ready to Start' : '🔒 Locked'}
              </span>
            </div>
      ...
      {unmetPrereqs.length > 0 && (
        <div className="rounded-xl border p-4 flex items-start gap-3"
          style={{ borderColor: 'hsl(var(--amber) / 0.4)', background: 'hsl(var(--amber) / 0.05)' }}>
          <span className="text-amber text-xl">⚠️</span>
          <div>
            <div className="font-medium text-amber">Prerequisites needed</div>
            <div className="text-sm text-muted-foreground mt-1">
              Complete {unmetPrereqs.map(p => p!.title).join(', ')} before this topic for the best experience.
            </div>
            <div className="flex gap-2 mt-2">
              {unmetPrereqs.map(p => (
                <button key={p!.id} onClick={() => navigate(`/curriculum/${p!.id}`)}
                  className="text-xs px-3 py-1 rounded-lg border border-amber/30 text-amber hover:bg-amber/10 transition-colors">
                  Start {p!.title}
                </button>
              ))}
            </div>
      ...
      {/* Main Content */}
      {!isLocked && (
        <Accordion type="multiple" defaultValue={['what', 'why', 'code', 'complexity']} className="space-y-3">
          {/* What is it */}
      ...
      {isLocked && (
        <div className="text-center p-8 rounded-xl border border-border"
          style={{ background: 'var(--gradient-card)' }}>
          <div className="text-4xl mb-3">🔒</div>
          <h3 className="font-semibold mb-2">Topic Locked</h3>
          <p className="text-muted-foreground text-sm mb-4">Complete prerequisites to unlock this topic</p>
          <Button variant="outline" onClick={() => navigate('/curriculum')}>View Curriculum</Button>
        </div>
      )}
    </div>
  );
